...
import { Download, CheckCircle, ChevronDown, Search, Loader2, X, Camera, Sun, Moon, FileText, Image, Map } from "lucide-react";
    ...
    switch (action.action) {
      case "update_map_view":
        if (action.lat !== undefined && action.lon !== undefined) {
          handleLocationSelect(action.lat, action.lon, action.name || `${action.lat.toFixed(4)}, ${action.lon.toFixed(4)}`);
        ...
        <div className="w-[300px] min-w-[260px] border-r border-border bg-card flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            {leftTab === "layers" ? (
              ...
              <ChatPanel location={selectedLocation} onToggleLayer={handleToggleLayer} activeLayers={activeLayers} onMapAction={handleMapAction} />
            )}
          </div>

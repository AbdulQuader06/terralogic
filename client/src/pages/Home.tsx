...
import { Download, CheckCircle, ChevronDown, Search, Loader2, X, Camera, Sun, Moon, FileText, Image, Map, Eye, EyeOff, Lock, Unlock, MapPin, Navigation, Upload, Box, PenTool } from "lucide-react";
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden flex-col">
      <header className="h-12 border-b border-border bg-card flex items-center px-4 justify-between z-10 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className="text-xs h-7 px-3 border-cyan-500/40 text-cyan-600 hover:bg-cyan-500/10"
            variant="outline"
            data-testid="button-bim-designer"
          >
          ...
          <Button
            ...
            className={`text-xs h-7 px-3 ${show3D ? "bg-primary hover:bg-primary/90 text-primary-foreground" : "border-primary/40 text-primary hover:bg-primary/10"}`}
            ...
            <Suspense fallback={
              <div className="absolute inset-0 z-[1000] bg-background flex items-center justify-center">
                <div className="text-center space-y-3">
                  ...
                  <div className="text-sm text-foreground">Loading 3D Engine...</div>
                </div>
              </div>
            }>
              <Map3DViewer location={selectedLocation} onClose={() => setShow3D(false)} />
            </Suspense>
          )}
        </div>
      ...
      <ImportModal
        ...
        onImport={handleCustomDataLoaded}
      />
    </div>
  );
}

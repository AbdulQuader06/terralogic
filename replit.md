...
-   **BIM-GIS Hybrid Designer**: Autonomous BIM design module at `/bim` route. Light/white theme matching the home page aesthetic (bg #EFF3F6, primary #2C5282, accent #2A9D8F). Three-panel layout: Left (Site Selection via Leaflet mini-map with Esri Topo basemap, Ctrl+Drag rectangle draw, amenity mix display, site summary, existing buildings list with height data), Center (Three.js 3D massing viewport with light background, navigate/place/select tools, ghost preview, interactive height sliders, orbit/pan/zoom, sun shadow study, color-coded existing buildings by type with height labels and hover tooltips), Right (tabbed panel with three tabs: Compliance — AI NBC analysis + radar chart + key metrics + investment estimates; Sun — interactive stereographic sun path diagram with time-of-day and season sliders, animated playback, altitude/azimuth/shadow ratio readouts; CartoAI — integrated chat assistant for BIM context). Features: click-to-place massing boxes (6 types), real-time FAR/ground-coverage/open-space computation, debounced AI compliance checking, PDF "Construction Brief" export with light theme. Components: `BimDesigner.tsx`, `SiteSelector.tsx`, `BimViewport.tsx`, `CompliancePanel.tsx`, `MetricsDashboard.tsx`, `SunAnalysis.tsx`. Backend: `POST /api/bim/compliance`.
